...
def generate_stats():
    ...
    # Liczba pojazdów na parkingu aktualnie
    cars_on_parking = len(cars_on_parking_registrations)

    # Liczba pojazdów na parkingu w ciągu ostatnich 24h
    cursor.execute("""
    SELECT COUNT(*) 
    FROM (
    ...
    # Całkowity czas przebywania pojazdów na parkingu
    cursor.execute("""
        WITH
            Wjazdy AS (
